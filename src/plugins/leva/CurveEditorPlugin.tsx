...
import { bakeCurveLUT, evaluateCurveLUT, type CurvePoint } from '../../curves.js'
import * as COLOR from '../../shared/constants/Color.js'
...
const CurveNumberInput = ({ id, label, value, settings, innerLabelTrim, onUpdate }: {
    ...
    const setValue = useCallback((nextValue: CurveNumberInputUpdate) => {
    ...
    const numberSettings = { ...settings, pad: settings.pad ?? 0, initialValue: value }
    const number = useInputSetters({ type: 'NUMBER', value, settings: numberSettings, setValue })
        ...
        <>
            ...
            <Row input>
                ...
                <CurveVectorInput
                    ...
                    innerLabelTrim={10}
                    onUpdate={(v: Record<string, number>) => onUpdate({ ...value, samples: v.samples ?? samples })}
                />
            </Row>
            <br />
            {points.map((p, i) => (
                            ...
                            <>
                                <Row input>
                                    ...
                                    <CurveVectorInput
                                        ...
                                        innerLabelTrim={4}
                                        onUpdate={(v: Record<string, number>) => {
...
type CurveNumberInputUpdate = number | ((previousValue: number) => number)
